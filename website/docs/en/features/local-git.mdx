...
<Shot
  id="local-git"
  alt="A merge request row selected in the table, and the footer reporting a completed checkout: platform slash ingest, branch feat slash retry-budget, fetched and switched. The preview pane notes that the local branch is three commits behind the source branch."
  caption="One key turns the selected row into a checked-out branch in the clone you mapped."
/>
