...
| L2 actions | 51 mnemonics, one for each verb | `?` and the command palette |
| L3 chords | `Ctrl+R` `Ctrl+A` `Ctrl+K` `Ctrl+P` `Ctrl+G` `Ctrl+T` `Ctrl+N` | This page |
...
| `]` | next preview tab |
| `Ctrl+A` | sign in again to this instance |
...
| --- | --- |
| `⏎` | continue |
| `o` | open the approval page again |
| `y` | copy the one-time code |
| `t` | paste a personal access token instead |
