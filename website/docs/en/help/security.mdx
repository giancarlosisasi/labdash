...
| A `--token <value>` flag | It would land in shell history and in the process list, visible to every user on the machine. A pipe, or the app's own token screen |
| A token typed at a terminal prompt | Reading with echo off needs raw mode, and raw mode swallows `Ctrl+C`. `auth login --with-token` reads a pipe only |
