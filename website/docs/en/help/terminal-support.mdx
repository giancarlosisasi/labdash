...
A Nerd Font is optional. Every glyph has an ASCII fallback of the same display width, so a screen rendered both ways has identical column boundaries.

Some multiplexer and font combinations report the wrong capability. Set it yourself when that happens:
