...
There is no Nerd Font, and detection did not notice. Force the fallback:
...
Columns line up identically. Every glyph has an ASCII replacement of the same display width.

### Columns are misaligned

Almost always a font that renders a glyph at a different width than it reports. `theme: { icons: ascii }` confirms it in one step: if the alignment fixes itself, the font is the cause.
