...
<Terminal
  ...
  cols={78}
  alt="A list of thirteen pipeline statuses, each shown with its Nerd Font glyph, its ASCII fallback of identical width, and its word."
...
A job that failed with `allow_failure: true` renders as a warning. Red is reserved for the failures that stop a pipeline.
...
Every Nerd Font glyph has an ASCII fallback of **the same display width**, so a screen rendered in either mode has identical column boundaries. A glyph that is one cell in one mode and two in the other is a layout bug.

`theme.icons` takes `auto`, `nerd` or `ascii`. `auto` detects a Nerd Font; the other two force the answer when detection is wrong. In `ascii` mode no byte above `0x7F` reaches the screen, borders are `+ - |`, and the spinner is `|/-\`.
